        ...
        async.parallel(
            ...
            (err, results, next) => {
                if (err) {
                  ...
                  if (!Bcrypt.compareSync(req.body.password_singin, results.user.password)) {
                    res.render("sign-in", {
                        user: req.body,
                        errors: ["Wrong password"],
                      });
                  } else {
                    const token = JsonWebToken.sign({ id: results.user._id, email: results.user.email}, SECRET_JWT_CODE);
